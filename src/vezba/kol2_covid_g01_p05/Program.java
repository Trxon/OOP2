...
public class Program {
	...
	public static void main(String[] args) {
		...
		datumi().stream().forEach(System.out::println);
		System.out.println(podaci(zemlje.get(0), LocalDate.of(2020, 5, 31)));
	}
	
	
	private static Podatak podaci(Zemlja zemlja, LocalDate datum) {
		return podaci.stream().filter(p -> p.getCountry().equals(zemlja) && p.getDate().equals(datum)).findFirst().orElse(null);
	}
	
	
	private static List<LocalDate> datumi() {
		return podaci.stream().map(Podatak::getDate).distinct().sorted().collect(Collectors.toList());
	}
	...
	private static void load() {
		...
		try (BufferedReader in = new BufferedReader(new InputStreamReader(Program.class.getResourceAsStream("sars-cov-2.csv")))) {
			...
			while ((line = in.readLine()) != null) {
				...
				if (tokens[10].equalsIgnoreCase("Other")) continue;
