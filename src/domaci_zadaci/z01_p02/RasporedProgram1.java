...
public class RasporedProgram1 {
	...
	private static void print(String[] lines, String name) {
		...
		for (String line : lines) {
			...
			if ("BEGIN".equalsIgnoreCase(propertyName) && "VEVEND".equalsIgnoreCase(propertyValue)) {
			 ...
			 */
				...
				if (values.length == 4 && values[0].equalsIgnoreCase(name) && values[2].trim().equals("(P)")) {
	...
	private static boolean checkTime(String value) {
		...
		try {
			if (value.charAt(8) != 'T') return false;
