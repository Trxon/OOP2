...
import domaci_zadaci.z01_ass.RasporedProgram1;
...
public class RasporedProgram1Regex {
	...
	private static void print(String lines, String unos) {
		...
		Pattern rrulePattern = Pattern.compile("(?sm)^RRULE(?:\\:.*?)*=(?:.*?=.*?)(?<dan>\\w{2})$");
		Pattern summaryPattern = Pattern.compile("(?sm)^SUMMARY(?:\\:.*?)*(?<predmet>.*?)\\\\,\\s*(?<nastavnik>.*?)\\\\,\\s*\\((?<tip>.*?)\\)\\\\,\\s*(?<sala>.*?)$");
		...
		while (eventMatcher.find()) {
			...
			if ("P".equalsIgnoreCase(casTip) && unos.equalsIgnoreCase(casPredmet))
	...
	private static void reportError(String message, Matcher matcher) {
		...
		Pattern idPattern = Pattern.compile("(?sm)^UID:(?<id>.*?)\\s*?$");
		...
		System.err.printf("%s, dogadjaj %s: %s%n \n", message, id, info); 
	}
}
