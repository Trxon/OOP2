package domaci_zadaci.z03_p00;

import java.time.LocalDate;
import java.util.Random;
import java.util.stream.Stream;

import domaci_zadaci.z03_p00.Osoba.Pol;

public final class Osobe {

	private static final String[] IMENA = new String[] {
			"Per", "Mik", "Djok", "Raj", "Gaj", "Vlaj", "Zlaj",
			"Ac", "Jov", "Kost", "Nikol", "Vlad", "Vukot", "Zivot", "Sinis", "Punis",
			"Nad", "Ned", "An", "Tanj", "Jelen", "Marij", "Tamar", "Nin", "Mil", "Mim",
	...
	protected static final Osoba osoba(Random r) {
		int index = r.nextInt(IMENA.length);
		String ime = IMENA[index] + "a";
		String prezime = IMENA[r.nextInt(IMENA.length)] + ((r.nextDouble() > 0.4) ? "ic" : r.nextDouble() > 0.5 ? "ovic" : "ovski");
		Pol pol = index < IMENA.length / 2 ? Pol.MUSKI : Pol.ZENSKI;
		if (pol == Pol.ZENSKI && r.nextDouble() < 0.1) {
			prezime = prezime + "-" + IMENA[r.nextInt(IMENA.length)] + "ic";
		}
		...
		Osoba[] deca = new Osoba[r.nextInt(4)];
